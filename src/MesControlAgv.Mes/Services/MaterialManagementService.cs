        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(sampleId))
        {
            ...
            sample ??= await _database.SampleMaterials.SingleOrDefaultAsync(
                ...
                cancellationToken);
            // Legacy custody records use a business sample ID, not the inventory GUID.
            // Preserve that separate lifecycle; never invent an inventory record or bypass
            // reservation when the same barcode already exists in the inventory ledger.
            var legacy = sample is null
                ? await _database.Samples.AsNoTracking().SingleOrDefaultAsync(
                    item => item.SampleId.ToUpper() == normalizedSampleId || item.Barcode.ToUpper() == normalizedSampleId,
                    cancellationToken)
                : null;
            if (sample is null && legacy is not null)
            {
                var legacyBarcode = Normalize(legacy.Barcode);
                sample = await _database.SampleMaterials.SingleOrDefaultAsync(
                    item => item.Barcode.ToUpper() == legacyBarcode, cancellationToken);
            }
            if (sample is null && legacy is null)
                throw Issue(
