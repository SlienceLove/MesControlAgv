...
- Task 5 的离线回归为 MES `224/224`、WPF（`--no-build`）`411/411`、WorkflowContract `71/71`、Simulator `5/5`；精确解决方案命令 `dotnet test MesControlAgv.sln -m:1` 在隔离完整副本中通过，汇总为 `1101 passed / 5 existing skipped / 0 failed`。Release 构建 `0` 警告、`0` 错误。该结果不替代下次上电实证。
