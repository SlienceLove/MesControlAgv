...
> 此文件保留 2026-09-16/17 的已有任务正式流程证据。中控模板准备、导入回读、双条码绑定和后续现场接续请从 [2026-09-18 最新交接](SAMPLE-WORKSTATION-CENTRAL-PREPARATION-HANDOFF-2026-09-18.md) 继续；旧文中的“厂家任务表导入尚未实现”是历史状态。
...
任何以后的厂家任务表导入只能创建/准备任务，不得启动设备；运行仍由独立准入动作触发。本交接记录时尚未实现厂家任务表上传、USB 扫码输入、last-scan 回传或远程停止；后续上传实现见页首最新交接指针。既有单次发令、Running 证据、Unknown 停止后续流程和只读恢复语义保持不变。完整 API、状态机、问题码和离线验收见 [中控接入说明](SAMPLE-WORKSTATION-CENTRAL-INTEGRATION.md#2026-09-17-样品任务行人工核对与运行准入门禁) 与 [样品核对验收](diagnostics/2026-09-17-sample-verification-acceptance.md)。

## 后续边界

- 厂家冷态自动初始化已验证；v2 已移除开盖分液专用人工门禁，WPF 通用人工确认能力仍保留。
- 本交接记录时，任务表导入和动态创建任务仍留待后续扩展，不加入当时的已有任务节点；后续任务级准备实现见页首最新交接。
- 手动停止继续暂缓；流程取消只阻止后续步骤，不向已执行设备动作发送停止命令。
- 与主工作区另一条四阶段工作站实现整合时，只保留本节点的“已有任务、单次启动、Running 证据、只读恢复”语义，不整批带入未真机验证的写操作。
- 本交接记录时尚未实现多步骤实验实体运行、厂家任务表导入、动态任务创建或远程停止；它们不属于本次单步骤验收，也不得由导入/准备动作隐式触发。后续已实现的导入仍遵守“不隐式启动”。
