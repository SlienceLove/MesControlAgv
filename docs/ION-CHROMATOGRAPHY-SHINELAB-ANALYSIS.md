...
### 9.1 路线选择状态 (DEFERRED-2026-08-26)

- 当前路线: RPA / 文件交接 (tier 2)
- COM / Modbus (tier 1): 第6.2节寄存器表来自旧版 DLL逆向, 未经厂家确认前禁用
- API: 不存在
- DB: 仅可只读探测，用于取结果 / 验证三列是否落库; 禁止写入序列行
- 触发条件: 自动化落地 + batch-11 结论落地
...
### 11.4 2026-08-18 受控写入抓包结果

新的 60 秒 USBPcap 会话确认了 5 次由 ShineLab 发出的 Modbus `0x06`
写操作，每次都收到设备原样回显且 CRC 有效：

| 时间（CST） | 写帧 | 当前证据支持的解释 |
| --- | --- | --- |
| 09:25:41.282 | `01 06 13 DA 02 BC AC 64` | LC 泵流量原始值 `700` |
| 09:25:41.601 | `01 06 15 7D 00 01 DC 1E` | 泵使能值 `1` |
| 09:25:50.906 | `01 06 13 DA 02 BC AC 64` | 同一流量值被再次写入 |
| 09:26:18.218 | `01 06 15 7C 00 02 CD DF` | 温控使能位掩码值 `2` |
| 09:26:18.510 | `01 06 13 89 0D AC 58 49` | 柱温原始值 `3500` |

`0x13DA` 已获得最强的动态关联证据：写入前两个候选流量寄存器为
`300/300`，写入 `700` 后先变为 `700/300`，随后稳定为 `700/700`。
结合静态代码的乘数线索，可确认该样机使用 `raw / 1000`，即
`700 = 0.700 mL/min`。这仍不代表安全范围、重复执行和异常恢复语义已确认。

现场操作员随后确认，抓包期间额外执行的 ShineLab 操作与泵使能、温控使能
和柱温设定三组写入相符。结合静态地址线索、设备回显和操作员时间线，这些
地址和值在当前样机/软件版本上的业务关联已经确认。所有帧目前仍只作为被动
观察证据；安全范围、重复执行、断线恢复和自动写入仍未验证，因此未授权由
中控或验证器重放。
