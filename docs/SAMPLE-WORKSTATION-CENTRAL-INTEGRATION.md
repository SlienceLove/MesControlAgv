...
### 2026-09-18 当前状态：中控任务准备已实现，待现场验收

隔离分支现已完成正式的“核对快照 → 读取模板 → 显式来源绑定/编辑目标 → 保存准备 → 显式导入并回读 → 写双条码 → 独立运行准入 → 带码单次启动”链路。准备记录冻结工作流、排程、核对 revision/hash、来源样品与模板位置、生成任务号、生成文件/hash 和每条来源→目标关系；保存准备不写设备，显式导入不启动设备。WPF“任务排程”页已有“工作站任务准备”面板。当前实现、操作步骤、路由、问题码及现场清单以 [2026-09-18 最新交接](SAMPLE-WORKSTATION-CENTRAL-PREPARATION-HANDOFF-2026-09-18.md) 为准，自动化证据见 [中控准备链验证](diagnostics/2026-09-18-workstation-central-preparation-validation.md)。

自动化已在真实 ASP.NET HTTP 路由、临时 SQLite 和内存工作站替身上覆盖两个来源、三个目标行、导入精确回读、双条码确认、独立准入、一次带码启动、先 Running 后 Completed 以及租约释放。它不是真机验收：本阶段没有向真实设备上传、写条码或启动，也没有修改现场进程。来源→目标是计划/准备溯源，不是逐孔执行传感；模板未使用的条码槽位 `IsUsedByTemplate=false`，不得表述为已分液来源。

下列“条码通讯”和“实际模板与导入回读”小节保留各自提交时的历史状态。其“尚待整合”文字不再代表当前代码状态；现场 V1.02 上传/回读/写码/运行验收仍然待办，现场导入和运行仍须分别按验收清单取得当次操作授权。隔离分支尚未合入日常分支 `feature/wpf-ui-layout-optimization`；用户已条件授权在真实验收成功后合并，验收成功即激活该既有授权，无需再取得笼统的合并批准，但合并前仍须重新检查目标分支头和用户改动。

### 2026-09-18 V1.02 大瓶条码通讯（历史提交状态）
...
本小节提交时，正式 worker 仍使用旧 taskNo-only 调用，未自动更新条码，也未将核对表行顺序隐式映射为
...
文件层提交时仅保留模块编码与坐标；业务样品ID及1/2号大瓶绑定当时尚待和正式核对快照、WPF导入操作整合。该整合现已在隔离分支完成，但仍未向真机上传、写码或启动，也没有修改现场进程。
...
`ImportTasksAsync(deviceId, fileName, Stream, cancellationToken)` 已于2026-09-18按实测模板和厂家客户端编码实现，输入流仍由调用方拥有；导入不隐式启动。此段原先记录的 MES 任务级门禁、来源绑定和 WPF 操作入口现已在隔离分支整合完成；但底层接口和自动化通过都不能替代 V1.02 真机现场验收。
