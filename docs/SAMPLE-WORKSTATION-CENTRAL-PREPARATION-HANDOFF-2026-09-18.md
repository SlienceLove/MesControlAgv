...
实际实验时**另建每孔 50 µL 的新任务模板**，保留原 `test` 不变。当前接口导出的原记录是每孔 1000 µL，且枪头坐标为 `(0,0)`：零值含义与解析兼容尚待确认，不能直接运行或猜改坐标。本机开盖分液服务也需切换/核对最新构建。位置、体积、耗液合计及创建步骤见 [test 模板与 50 µL 约定](diagnostics/2026-09-20-workstation-test-template-50ul-plan.md)。本次尚未创建/导入新任务，也未启动设备。
...
1. 确认最新厂家服务及中控服务版本、可用设备和实际装载样品；先处理上文 `test` 的枪头 `(0,0)` 兼容点，再按已确认的 16 孔、每孔 50 µL 方案创建新任务，记录版本、时间与操作者。
