...
保留一项 Minor：`ExperimentSchedulingView.xaml` 中“人工放置”区的行高顺序目前为 `Auto, Auto, Auto, *`，应恢复原来的 `Auto, Auto, *, Auto`。资源较多时，资源列表可能挤占排程按钮空间；这不影响已排程任务的准备、导入及准入逻辑，但在多资源人工排程验收/后续合并前应恢复原布局并补相应测量断言。本轮没有据此宣称所有界面场景均已验收。
