...
- 最新界面实现头：`18e274a`（准备页统一体积、计划用量汇总、未保存提示）；短号兼容为 `0f31868`，默认枪头兼容为 `09190d0`，布局收尾为 `34bb395`。这些后续修正尚未部署现场运行服务。
...
## 2026-09-20 最新现场模板与用户约定

后续以厂家小写 `test` 为来源模板，不再沿用旧 `TEST-001` 单来源示例。用户已明确 `CYC-001-1000` 对应 1 号大瓶、`CYC-002-1000` 对应 2 号大瓶；两者分别对应 `FYB-001`、`FYB-002` 的 8 个目标孔位，共 16 条。

实际实验时**另建每孔 50 µL 的新任务模板**，保留原 `test` 不变。原记录每孔 1000 µL、枪头坐标 `(0,0)`；用户已明确枪头由工作站默认管理，中控不选择枪头。现已实现完整零坐标原样往返、保留其他孔位规则，并生成两来源16孔50µL的本地预览文件，见[兼容与预览验证](diagnostics/2026-09-20-workstation-zero-tip-preview-validation.md)。本机开盖分液服务仍需切换/核对最新构建。位置和创建约定见 [test 模板与 50 µL 约定](diagnostics/2026-09-20-workstation-test-template-50ul-plan.md)。预览不是正式准备记录；本次没有向厂家导入、写码或启动。

## WPF 操作步骤

### 不依赖厂家的界面提效（A）

用户确认后已完成统一每孔体积按钮、来源/目标孔/计划用量汇总与未保存提示。输入默认50µL但不自动应用；点击“应用到全部行”只改本地表格，仍需显式保存/导入/准入。见[界面提效与离线验证](diagnostics/2026-09-20-workstation-preparation-editing-validation.md)。未打开或替换现场WPF，不代表厂家导入已恢复。
...
1. 确认最新厂家服务及带零坐标兼容的中控服务版本、可用设备和实际装载样品；枪头沿用工作站默认值，按已确认的 16 孔、每孔 50 µL 方案创建正式新任务，记录版本、时间与操作者。
