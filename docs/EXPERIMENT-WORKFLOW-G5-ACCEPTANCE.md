...
> 状态：G5-A、G5-B、G5-C 已通过项目方验收；G5-D 实现与自动化门禁已完成，待项目方验收。
...
| G5-D | 独立 WPF 方案页、排程页、资源泳道和人工验收 | 实现完成，待项目验收 |
...
通过 G5-D 项目验收后，再确认 G5 总体验收。未获得确认前不进入 G6；自动调度、自动节点推进、实体
设备控制、串口访问、协议/寄存器字段和 CIC-D160+ 写入继续保持关闭。
