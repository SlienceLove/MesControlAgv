...
> 状态：G5-A 已实现并通过自动化门禁，等待项目方确认；G5-B、G5-C、G5-D 尚未开始。
...
| 切片 | 责任 | 当前状态 |
| --- | --- | --- |
| G5-A | 方案、任务、排程条目、计划预留和运行租约的契约、持久化及只读投影 | 已实现 |
| G5-B | 方案/任务生命周期、人工排程、确定性预留冲突、阻塞原因和审计 | 未开始 |
...
确认后才可进入 G5-B。G5-B 不得提前接入运行租约，不得由排程器推进流程节点，
