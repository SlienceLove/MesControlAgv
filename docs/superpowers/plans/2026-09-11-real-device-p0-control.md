...
## 7. Task 6：两台离子色谱正式模块接入（D0-D9，关键路径）
...
- 在闸门关闭期间，只能完成双设备只读身份/状态和报文捕获分析；不得把 fallback 编号或推测字段接入生产控制。
- D2 结束仍未交付时，项目负责人必须选择延期或明确接受“其余三类真实执行 + 离子色谱只读”的降级范围；不能将降级结果标为四类全通过。
...
**关键升级规则**：D2 离子色谱厂家闸门未关闭，立即向项目负责人升级；D9 任一离子设备仍只有只读能力，不能宣称“四类设备真实控制完成”；D12 现场预演出现 Unknown 未完成对账，D14 不得重发同一动作，只能换新运行 ID 并由人工确认安全状态后决定是否继续。
...
- 风险结论：若厂家协议未按时交付，单独列为 P0 阻塞，不使用 Simulator、历史截图或只读状态替代。
