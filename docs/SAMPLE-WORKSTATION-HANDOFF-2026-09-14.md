...
剩余重点：中控实现/对齐发令前二次确认和取消，厂家补齐异常/手动停止时的任务终态，后续依据厂家样表接入任务导入。中控 UI 确认流程、批量导入和分液精度尚未验收。
...
1. **异常与手动停止终态**：1625 版正常运行期间 `GetInstrumentStatus` 已实测返回 `1/Running`，结束回到 `0/Idle`。但静态代码中结果 1、2、-1～-4 没有像成功分支一样调用 `Update_TaskData()`，任务可能继续停留 Running；暂停 2、故障 3、初始化 4 也未主动测试。当前不在中控伪造终态。
