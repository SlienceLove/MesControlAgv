...
剩余重点：厂家修设备总状态，中控实现/对齐发令前二次确认和取消，后续依据厂家样表接入任务导入。中控 UI 确认流程、批量导入和分液精度尚未验收。
...
1. **厂家设备总状态回写**：运行期间 `GetInstrumentStatus` 仍返回 `0/Idle`，应为 `1/Running`，结束回到 `0`。暂停 2、故障 3、初始化 4 按实际状态核对，但未主动测试这些分支。不要在中控强行改写厂家状态掩盖问题，也不要只凭该错误的 Idle 值允许下一个任务。
2. **中控最小确认/取消接入**：对齐现有页面/工作流入口，发令前显示确认；取消请求数为 0，确认请求数为 1；显示“已发出/运行/完成”的实际区别。先用模拟响应做离线验证，不添加复杂状态机、审计框架或自动重试。
3. **取得最终厂家程序和匹配 DLL**：16:44 现场再次更新后的无弹窗版本未取得新的文件哈希。`res/DLHWorkstation_最新.exe` 的已核对 SHA 是 16:20 的旧快照，不应据此断言现场仍有弹窗。需要新包时向用户索取，别修改厂家二进制。
