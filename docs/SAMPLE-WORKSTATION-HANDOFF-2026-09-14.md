...
2026-09-16 已完成 WPF 真机验收：厂家主程序手动完整初始化后，WPF 选择 `TEST-001`、二次确认、单次启动、Running→Completed 全部通过。前两次未完整初始化时没有进入运行，WPF 正确拒绝将旧 Completed 当作本轮完成，也没有自动重发。详见 [WPF 真机验收记录](diagnostics/2026-09-16-workstation-wpf-field-validation.md)。代码仍位于隔离分支，尚未合入主工作区。

剩余重点：转入正式完整工作流节点与 MES worker 接入；后续依据厂家样表接入任务导入。厂家冷启动自动初始化仍需补齐，在修复前现场需先从厂家主程序执行完整初始化。批量导入和分液精度尚未验收。手动停止功能目前不纳入中控接入范围，只保留为后续规划项；本阶段不实现、不测试，也不要求厂家为此新增远程停止接口。
...
真机 WPF 验收已于 2026-09-16 完成。三次用户确认对应三条厂家启动请求；前两次因未完整初始化未运行，第三次在厂家主程序手动初始化后于 08:53:50 进入 Running、08:56:20 完成，用户确认 WPF 显示“任务完成”。本轮本机 15041/15045 与 WPF 测试实例在归档后关闭；厂家 192.168.200.157:8082 服务未被关闭。
