...
> 状态：G4-A、G4-B、G4-C、G4-D 及 G4 总体已通过项目方验收；项目方已授权进入 G5。
...
提交：`379fd59 feat(workflows): add audited run controls`
...
项目方 G4-D 及 G4 总体验收结论：**通过**。

最终人工证据：

- 取消运行 `5723597b-c4c6-4d8c-9a81-eb534c17492d` 为 Cancelled；节点为 Succeeded、Cancelled，设备操作仍为 0。
- Unknown 成功运行 `07984ede-c82a-4605-8567-44451dc8ba2c` 为 Completed；节点和原设备操作为 Succeeded，设备操作总数仍为 1。
- Unknown 失败运行 `42b30280-65b9-426e-bce5-81723fc62062` 为 Failed；节点和原设备操作为 Failed，设备操作总数仍为 1。
...
- G5 的方案、排程和资源租约不属于 G4；G5 应继续遵守本节设备控制边界。
