...
- 使用新任务号及本地预览名称 `LOCAL-PREVIEW-50uL`；每条50µL，各来源合计400µL，总计800µL。原始1000µL输入未修改，枪头默认值及除体积之外的转移字段/顺序保持原样。
...
实现提交：`09190d0`。独立只读审查覆盖 `f5594f8..09190d0`，未发现代码阻断问题；发现一项名称说明偏差和一项历史进度措辞过时，均已修正文档：明确新任务号/本地预览名称与体积发生变化，其他转移字段不变，并区分初次整理与后续实现。未修改已验证代码或覆盖预览文件。最终归档仍在隔离功能分支，真实导入、运行验收及日常分支合并尚未进行。
