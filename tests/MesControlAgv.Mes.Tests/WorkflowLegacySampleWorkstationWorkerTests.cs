...
public sealed class WorkflowLegacySampleWorkstationWorkerTests
{
    [Theory]
    [InlineData("null")]
    [InlineData("")]
    ...
    public async Task Unreadable_query_payload_is_a_read_error_not_a_command_outcome(string payload)
    {
        ...
        var reader = new LegacySampleWorkstationAdapterClient(client);
        ...
        string scenario, bool succeeds, int expectedWrites)
    {
        ...
        var dispatcher = new WorkflowLegacySampleWorkstationDispatcher(workflows, gateway, gateway, profile,
            new WorkflowLegacySampleWorkstationWorkerOptions
            {
    ...
    private sealed class InMemoryWorkstation(AdjustableTimeProvider clock, string scenario) : ISampleWorkstationController, ISampleWorkstationReader
    {
        public Task<SampleWorkstationProtocolResponse> GetProtocolReadAsync(
            ...
            throw new NotSupportedException("Legacy execution must not use generic protocol reads.");
